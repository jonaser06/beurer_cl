    ...
    document.querySelector('.formPayu').addEventListener('submit', async e => {
        ...
        e.target.innerHTML += `
                    ...
                    <input name="displayShippingInformation"    type="hidden"  value="${parseInt(envio)? 'YES':'NO'}">
                     ${$inputRecojo}
                    <input name="confirmationUrl" type="hidden" value="${DOMAIN}burer_plantilla/payu.php">
                    <input name="responseUrl" type="hidden"  value="${DOMAIN}ajax/confirmPagePayu?payu=true&session=${session}&${ADD_PARAMS()}${ADD_QUERY_PARAMS()}">`;
